...
final class PdoNotificationRepository implements NotificationRepository
{
    ...
    public function updatePreferences(int $householdId, int $userId, ?bool $taskDue, ?bool $inventoryExpiry): void
    {
        ...
        $statement = $this->pdo->prepare(
            'INSERT INTO jarvis_notification_preferences (household_id, user_id, task_due, inventory_expiry) VALUES (:household_id, :user_id, :task_due, :inventory_expiry) '
    ...
    {
        $statement = $this->pdo->prepare(
            "UPDATE jarvis_notification_jobs SET status = 'cancelled', last_error = NULL, updated_at = CURRENT_TIMESTAMP(6) "
    ...
    public function upsertJob(int $householdId, int $userId, string $eventKey, string $jobType, string $scheduledAt, array $payload): void
    {
        ...
        $statement = $this->pdo->prepare(
            'INSERT INTO jarvis_notification_jobs (household_id, user_id, event_key, job_type, scheduled_at, payload_snapshot) '
    ...
    public function expiryRecipients(string $fromDate, string $throughDate): array
    {
        $statement = $this->pdo->prepare(
            ...
            . 'FROM jarvis_household_members hm '
            . 'LEFT JOIN jarvis_notification_preferences p ON p.household_id = hm.household_id AND p.user_id = hm.user_id '
            . 'JOIN jarvis_inventory_batches b ON b.household_id = hm.household_id AND b.quantity > 0 AND b.expires_on BETWEEN :from_date AND :through_date '
            . 'JOIN jarvis_ingredients i ON i.household_id = b.household_id AND i.id = b.ingredient_id '
    ...
    public function claimDueJob(string $nowUtc): ?array
    {
        $statement = $this->pdo->prepare(
            ...
            . 'FROM jarvis_notification_jobs j JOIN jarvis_users u ON u.id = j.user_id '
            . 'LEFT JOIN jarvis_notification_preferences p ON p.household_id = j.household_id AND p.user_id = j.user_id '
    ...
    public function taskJobIsCurrent(array $job): bool
    {
        ...
        $payload = $job['payload'];
        $statement = $this->pdo->prepare(
            "SELECT 1 FROM jarvis_tasks WHERE household_id = :household_id AND id = :task_id AND status = 'pending' "
    ...
    {
        $statement = $this->pdo->prepare(
            "SELECT id FROM jarvis_notification_grants WHERE household_id = :household_id AND user_id = :user_id AND template_type = :template_type AND status = 'available' ORDER BY id LIMIT 1 FOR UPDATE"
        );
        $statement->execute(['household_id' => $householdId, 'user_id' => $userId, 'template_type' => $templateType]);
