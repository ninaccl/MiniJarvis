...
final class PdoInventoryRepository implements InventoryRepository
{
    ...
    public function createBatch(int $householdId, int $userId, array $batch): int
    {
        $statement = $this->pdo->prepare(
            'INSERT INTO inventory_batches (household_id, ingredient_id, quantity, unit_code, display_quantity, display_unit_code, expires_on, note, created_by) '
    ...
    {
        $statement = $this->pdo->prepare('UPDATE inventory_batches SET quantity = :quantity, updated_at = CURRENT_TIMESTAMP(6) WHERE household_id = :household_id AND id = :id');
    ...
    {
        $statement = $this->pdo->prepare(
            'INSERT INTO inventory_movements (household_id, batch_id, ingredient_id, movement_type, quantity, unit_code, display_quantity, display_unit_code, actor_user_id, note) '
            ...
            . 'FROM inventory_batches b WHERE b.household_id = :household_id AND b.id = :batch_id'
        );
        $statement->execute([
    ...
    {
        $count = $this->pdo->prepare('SELECT COUNT(*) FROM inventory_movements WHERE household_id = :household_id');
        ...
        $statement = $this->pdo->prepare(
            ...
            . 'FROM inventory_movements m JOIN ingredients i ON i.household_id = m.household_id AND i.id = m.ingredient_id '
    ...
    private function batchSelect(): string
    {
        return 'SELECT b.id, b.household_id, b.ingredient_id, i.name AS ingredient_name, '
            ...
            . 'FROM inventory_batches b JOIN ingredients i ON i.household_id = b.household_id AND i.id = b.ingredient_id';
    }
