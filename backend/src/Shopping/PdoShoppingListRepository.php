...
final class PdoShoppingListRepository implements ShoppingListRepository
{
    ...
    {
        $statement = $this->pdo->prepare('INSERT INTO shopping_lists (household_id, name, status, selection_snapshot, created_by) VALUES (:household_id, :name, \'active\', :selection_snapshot, :created_by)');
        $statement->execute([
    ...
    public function addItem(int $householdId, int $listId, array $item): int
    {
        $statement = $this->pdo->prepare(
            'INSERT INTO shopping_list_items (shopping_list_id, household_id, ingredient_id, name, required_quantity, inventory_offset, quantity, unit_code, source_summary) '
    ...
    {
        $count = $this->pdo->prepare('SELECT COUNT(*) FROM shopping_lists WHERE household_id = :household_id');
    ...
    {
        $statement = $this->pdo->prepare(
            'UPDATE shopping_list_items SET is_checked = :checked, checked_household_id = :checked_household_id, checked_by = :checked_by, checked_at = :checked_at, updated_at = CURRENT_TIMESTAMP(6) '
    ...
    {
        $statement = $this->pdo->prepare(
            'UPDATE shopping_lists SET status = :status, completed_at = IF(:completed = 1, COALESCE(completed_at, CURRENT_TIMESTAMP(6)), NULL), updated_at = CURRENT_TIMESTAMP(6) '
    ...
    {
        $statement = $this->pdo->prepare(
            'UPDATE shopping_list_items SET stocked_at = CURRENT_TIMESTAMP(6), stocked_household_id = :stocked_household_id, stocked_by = :stocked_by, stocked_batch_id = :batch_id, updated_at = CURRENT_TIMESTAMP(6) '
    ...
    private function listSelect(): string
    {
        return 'SELECT l.id, l.household_id, l.name, l.status, l.selection_snapshot, l.created_by, l.created_at, l.updated_at, l.completed_at, '
            . '(SELECT COUNT(*) FROM shopping_list_items i WHERE i.household_id = l.household_id AND i.shopping_list_id = l.id) AS item_count, '
            . '(SELECT COUNT(*) FROM shopping_list_items i WHERE i.household_id = l.household_id AND i.shopping_list_id = l.id AND i.is_checked = 1) AS checked_count FROM shopping_lists l';
    }
    ...
    private function itemSelect(): string
    {
        return 'SELECT i.id, i.shopping_list_id, i.ingredient_id, i.name, i.required_quantity, i.inventory_offset, i.quantity, i.unit_code, i.source_summary, '
            . 'i.is_checked, i.checked_by, i.checked_at, i.stocked_at, i.stocked_by, i.stocked_batch_id, i.created_at, i.updated_at FROM shopping_list_items i';
    }
